...
#define WRAP( fn, ret, ... ) ret wrap_##fn(__VA_ARGS__)
...
static bool Descend( char *pPath, size_t nStartIdx, bool bAllowBasenameMismatch, size_t nLevel = 0 )
{
	...
	if ( access( CDirTrimmer(pPath, nNextSlash), F_OK ) == 0 )
	{
		if ( !bIsDir )
			return true;
	...
	{
		// we have a path
		spDir = opendir( CDirTrimmer( pPath, nStartIdx ) );
		nStartIdx++;
	}
	else
	{
...
extern "C" {
	...
	{
		return wrap_open( pathname, O_CREAT|O_WRONLY|O_TRUNC, mode );
	}

	int wrap_access(const char *pathname, int mode)
	{
		return access( CWrap( pathname, false ), mode );
	}
    ...
    {
        return CALL(stat)( CWrap( __filename, false), __stat_buf );
    }

    WRAP(__lxstat, int, int __ver, __const char *__filename, struct stat *__stat_buf)
    {
        return CALL(lstat)( CWrap( __filename, false), __stat_buf );
    }
	...
	WRAP(rmdir, int, const char *pathname)
	{
